...
- **`blogshot=SUCCEEDED`는 실행 성공일 뿐 게시 승인으로 보지 않습니다.** artifact의 실제 이미지를 열어 본문의 특정 주장과 직접 연결되는지, 핵심 정보가 잘리지 않았는지, 불필요한 영역이나 경고가 섞이지 않았는지 검수합니다.
- 이미지가 내용상 부적절하거나 잘린 경우 commit하지 않고 selector/viewport를 조정해 다시 캡처하거나, 텍스트·코드·Mermaid 등 더 적합한 표현으로 대체합니다.
- 게시하는 이미지에는 의미 있는 `alt`와 독자가 무엇을 봐야 하는지 설명하는 캡션을 붙입니다.
- 이미지 반영 후에는 GitHub Pages 빌드 성공만 확인하지 않고 **실제 렌더링된 게시물 문맥에서 크기·가독성·잘림·주변 문단과의 연결성**을 다시 확인합니다.
...
5. 민감정보가 제거되어 있다.
6. 이미지가 있다면 실제 이미지를 열어 내용 적합성·완전한 구도·불필요 영역·공개 안전성을 검수했고, Pages에서 렌더링된 문맥도 확인했다.
7. 이력서/포트폴리오에서 링크해도 부끄럽지 않은 수준이다.
8. 제목만 봐도 어떤 역량을 보여주는 글인지 알 수 있다.
