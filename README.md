...
이 블로그에서 가장 상위에 두는 기준은 **실제 문제를 해결하고, 만든 결과가 제대로 작동하는지 확인하는 것**입니다.

개발, 보안, 데이터, 아키텍처, AI는 그 목적을 위한 수단으로 다룹니다. 특히 AI는 커리어의 목적이 아니라 현재 문제 해결 범위를 확장하기 위해 사용하는 도구입니다.

자세한 관점은 `_posts/2026-09-20-ai-is-a-tool-for-problem-solving.md`와 `about.md`에 정리합니다.
...
홈 화면은 최신 글 피드보다 작성된 기술·업무 글의 주제별 목록을 우선합니다.

주택, 생활계획, 개인 장기 의사결정처럼 기술 포트폴리오와 직접 관련되지 않는 공개 기록은 `Personal` 태그를 사용하고 `/notes/` 목록에서 별도로 관리합니다. 기존 `Housing` 태그도 Personal Notes로 취급합니다.

따라서 개인 기록은 다음 위치에서 제외합니다.

- 홈의 기술·업무 글 목록
- `/archive/` 기술 글 목록
- `/categories/` 기술 카테고리 목록

## Category Policy

카테고리는 글의 큰 목적을 나타내고, 태그는 세부 기술과 키워드를 나타냅니다.

기술·업무 카테고리는 아래 7개로 고정합니다.
...
| `career` | 이직, 포트폴리오, 블로그 운영, AI 개발 워크플로, 경력 방향 |

`study`, `linux`, `spring`, `pmp`, `cisa`, `troubleshooting`, `personal`, `housing`은 기술 카테고리로 쓰지 않고 태그로 사용합니다.
...
- Career: 이직, 포트폴리오, 기술 블로그 운영, AI 개발 워크플로
- Notes: 주택, 생활계획 등 개인 의사결정 기록
...
-> 핵심 내용 추출
-> 공개 가능 여부 검토
-> 기술/업무 글인지 Personal Notes인지 분류
-> 카테고리/태그 지정
-> Markdown 글 작성
-> GitHub repo에 commit/push
-> GitHub Pages로 공개
-> 필요하면 이력서/포트폴리오에 주요 기술 글 링크 연결
```
...
포트폴리오에는 주요 기술 글을 프로젝트별 근거 자료로 연결합니다.
...
Personal Notes는 기본적으로 이력서/포트폴리오 근거로 사용하지 않습니다.

## Operating Principle

글을 많이 쓰는 것보다, 실제 이력서와 면접에서 설명 가능한 기술 글을 축적하는 것을 우선합니다.

각 기술 글은 다음 질문에 답해야 합니다.
